...
void* kmalloc(uint32 size) {
	...
	if (!ptr) {
		...
		dkprintf("EXTENDING KMALLOC HEAP _size:%x\n", _size);
...
void kdumpthreadinfo(KTHREAD *th) {
	dkprintf("r0:%x\tr1:%x\tr2:%x\tr3:%x\n", th->r0, th->r1, th->r2, th->r3);
	dkprintf("r4:%x\tr5:%x\tr6:%x\tr7:%x\n", th->r4, th->r5, th->r6, th->r7);
	dkprintf("r8:%x\tr9:%x\tr10:%x\tr11:%x\n", th->r8, th->r9, th->r10, th->r11);
	dkprintf("r12:%x\tsp:%x\tlr:%x\tcpsr:%x\n", th->r12, th->sp, th->lr, th->cpsr);
}
...
void ksched(KTHREAD *tothread) {
	...
	*/
	...
	if (ks->holdcpus == 1) {
		for (;;) {
			dkprintf("WARNING cpu-holding-lock:%x\n", ks->schedlock.lock);
			for (;;);
		}
	}
			...
			for (th = proc->threads; th; th = th->next) {
				...
				if (th->flags & KTHREAD_SLEEPING) {
					...
					if (th->flags & KTHREAD_WAKEUP) {
						dkprintf("WOKE UP (signal) thread:%x dbgname:%s\n", th, th->dbgname ? th->dbgname : "<unknown>");
	...
	*/
	...
	dkprintf("SWITCH-TO thread:%x cpsr:%x fp:%x sp:%x pc:%x dbgname:%s\n", kt, kt->cpsr, kt->r11, kt->sp, kt->pc, kt->dbgname);
	
	//uint32			*p;
	
	//if (!kvmm2_getphy(&cs->cproc->vmm, 0x90000000, (uintptr*)&p)) {
		//dkprintf("NO STACK EXISTS??\n");
	//} else {
		//dkprintf("writing to stack..%x\n", kt->sp);
		//((uint32*)kt->sp)[-1] = 0xbb;
	//}
...
static void kprocfree__walkercb(uintptr v, uintptr p) {
	...
	switch (r0) {
		case 1:
			...
			dkprintf("    no references (but alloced) %x:%x\n", v, p);
			kvmm2_revdec(p);
		...
		case 0:
			...
			break;
		default:
			dkprintf("     decrement ref cnt for %x:%x\n", v, p);
...
void k_exphandler(uint32 lr, uint32 type) {
	...
	//asm("mov %[var], sp\n" : [var]"=r" (r0));
	//dkprintf("SPSPSPSP:%x type:%x\n", r0, type);
	
	if (type == ARM4_XRQ_FIQ) {
		for (;;) {
			dkprintf("PANIC: NOT EXPECTING FIQ\n");
		}
	}
	...
	if (type == ARM4_XRQ_IRQ) {
		if (kboardCheckAndClearTimerINT()) {			
			...
			ks->ctime += r0;
			//dkprintf("delta:%x ks->ctime:%x ks:%x\n", r0, ks->ctime, ks);
			//dkprintf("IRQ: ctime:%x timer-tick:%x\n", ks->ctime, kboardGetTimerTick());
			ksched(0);
	...
	*/
	if (type == ARM4_XRQ_SWINT) {
		...
		//dkprintf("SWI thread:%x name:%s code:%x lr:%x\n", cs->cthread, cs->cthread->dbgname, swi, lr);
		...
		switch (swi) {
				...
				*/
				...
				for (x = 0; x < r1; ++x) {
					...
					switch (r0) {
						case 1:
							/* no references left (decrement to zero) */
							dkprintf("    no references (but alloced) %x\n", r0 + x * 0x1000);
							kvmm2_revdec(out);
						...
						case 0:
							...
							break;
						default:
							dkprintf("     decrement ref cnt for %x\n", r0 + x * 0x1000);
							/* decrement reference count */
							kvmm2_revdec(out);
							break;
					}
					...
					if ((uint32)proc == r0) {
						for (th = proc->threads; th; th = th->next) {
							if ((uint32)th == r1) {
								...
								dkprintf("mla adding signal tprocess:%x tthread:%x signal:%x\n", proc, th, r2);
					...
					if ((uint32)proc == r0) {
						for (th = proc->threads; th; th = th->next) {
							if ((uint32)th == r1) {
								ksched((KTHREAD*)r1);
								/*
									TODO: may need to clear WAKEUP flag
										  also, but just going to leave
										  it if it is set.. thread just
										  might wake up prematurely but
			...
			case KSWI_TKADDTHREAD:
				...
				r0 = stk[-14];
				r1 = stk[-13];
				dkprintf("SWI ADDTHREAD proc:%x thread:%x\n", r0, r1);				
				KCCENTER(&ks->schedlock);
			...
			case KSWI_KERNELMSG:
				...
				dkprintf("SYSCALL KERNELMSG\n");
				break;
			case KSWI_TKMALLOC:
				/* TODO: check for privilege mode */
				((uint32 volatile*)stk)[-14] = (uint32)kmalloc(stk[-14]);
				dkprintf("KSWI_TKMALLOC %x\n", stk[-14]);
				break;
...
KTHREAD* kelfload(KPROCESS *proc, uintptr addr, uintptr sz) {
	...
	dkprintf("@@ loading elf into memory space\n");
	...
	if (ehdr->e_machine != EM_ARM) {
		dkprintf("kelfload: not ARM machine!\n");
		return 0;
	}
	
	if (ehdr->e_ident[4] != 0x1) {
		dkprintf("kelfload: not ELF32 object\n");
		return 0;
	}
	...
	*/
	...
	for (x = 0; x < ehdr->e_shnum; ++x) {
		...
		if (shdr->sh_addr != 0) {
			...
			*/
			dkprintf("elf-load    virtual:%x size:%x\n", shdr->sh_addr, shdr->sh_size);
...
int kthread_threadverify(KPROCESS *tarproc, KTHREAD *tarth) {
	...
	dkprintf("[kthread] threadverify iterating process/thread chains\n");
	y = 1;
	while (y) {
		...
		for (proc = ks->procs; proc; proc = proc->next) {
			if (!proc || (proc->flags & KPROCESS_DEAD)) {
				y = 1;
				dkprintf("[kthread] iteration RESET\n");
				break;
			}
...
void kthread(KTHREAD *myth) {
	...
	for (;;) {
		dkprintf("[kthread] deallocing on MWSR\n");
		...
		dkprintf("[kthread] checking MWSRGLA\n");
		...
		while (mwsrgla_get(&ks->ktsignal, &mwsrglab, &mwsrglandx, (uintptr*)&th)) {
			...
			dkprintf("[testuelf] [kthread] reading RB for thread:%x[%s]\n", th, th->dbgname);
			for (sz = sizeof(pkt); er_read_nbio(&th->ktx, &pkt[0], &sz); sz = sizeof(pkt)) {
				dkprintf("[kthread] got pkt\n");
				dkprintf("[testuelf] [kthread] got pkt %x\n", pkt[0]);
				/* check packet type */
				switch (pkt[0]) {
					case KMSG_SENDMESSAGE:
						dkprintf("[kthread] send message to process:%x thread:%x\n", pkt[2], pkt[3]);
						...
						if (kthread_threadverify((KPROCESS*)pkt[2], (KTHREAD*)pkt[3])) {
							...
							dkprintf("[kthread] send failed - no proc/thread match\n");
					...
					case KMSG_CREATETHREAD:
						/* fill default stuff */
						dkprintf("[kthread] create thread message RID:%x pc:%x\n", pkt[1], pkt[2]);
					...
					case KMSG_REGSERVICE:
						...
						if (!(th->proc->flags & KPROCESS_SYSTEM)) {
							...
							dkprintf("[kthread] service registered for %x as %x:%x\n", pkt[2] & 3, pkt[3], pkt[4]);
					...
					case KMSG_REQSHARED:
						...
						dkprintf("[kthread] KMSG_REQSHARED\n");
						for (x = 0; x < 13; ++x) {
							dkprintf("[kthread] %x:%x\n", x, pkt[x]);
						}
						/* do verification (to prevent expoiting) */
						if (kthread_threadverify((KPROCESS*)pkt[4], (KTHREAD*)pkt[5])) {
							dkprintf("[kthread] verify good sending requent and reply\n");
							dkprintf("[kthread] RIQ:%x\n", pkt[1]);
							th->shreq_rid = 	pkt[1];
							...
							dkprintf("[kthread] req shared failed; sending reply\n");
					...
					case KMSG_ACPSHARED:
						...
						break;
					default:
						dkprintf("[kthread] unknown message type:%x size:%x\n", pkt[0], sz);
						break;
				}
			}
			dkprintf("[kthread] done reading RB\n");
		}
		dkprintf("[kthread] sleeping\n");
		ksleep(ks->tpers * 100);
	}
}
...
void start() {
	...
	*/
	...
	x = 0;
	dkprintf("looking at attached modules\n");
	for (m = kPkgGetFirstMod(); m; m = kPkgGetNextMod(m)) {
		dkprintf("looking at module\n");
		...
		dkprintf("looking for NEXT module..\n");
	}
	
	//dkprintf("....\n");
		
	dkprintf("kboardPostPagingInit()\n");	
